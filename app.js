...
orm.connect("mysql://root:@localhost/group6", (err, db) => {
    if (err) {
        ...
        app.post("/login", (req, res) => {
            ...
            models.login.get(user, (err, result) => {
                if (err) {
                    ...
                    if (result.password === pass) {
                        ...
                        if (result.officer_id) {
                            models.officers.get(result.officer_id, (err, result) => {
                                if (result.fname) {
                            ...
                            models.customers.get(result.customer_id, (err, result) => {
                                if (result.fname) {
                                    req.session.user.name = result.fullname()
                                }
                                redirect(req.session.user, res)
        ...
        app.get("/admin", (req, res) => {
            if (checkUser(req.session, res, 'admin')) {
                var officers = []
                var count = 0
                models.officers.find(true, (err, result) => {
                    count = result.length
                    if (count === 0) {
                        ...
                        result.forEach(e => {
                            ...
                            models.login.find({ officer_id: e.id }, (err, result) => {
                                if (err) {
                                    res.sendStatus(403)
                                } else {
                                    if (result.length === 0) {
        ...
        app.post("/admin/createOfficer", (req, res) => {
            ...
            models.officers.create({}, (err, result) => {
                const id = result.id
                if (err) {
        ...
        app.post("/editOfficer", (req, res) => {
            ...
            models.officers.get(req.session.user.officer_id, (err, result) => {
                if (err) {
                    ...
                    result.save((err, result) => {
                        if (err) {
                            ...
                            ((req.session.user.position)?models.debts:models.crms).create({
        ...
        app.get(["/crm", "/crm/customerList"], (req, res) => {
            if (checkUser(req.session, res, 'crm')) {
                models.customers.find(true, (err, result) => {
                    var customers = []
                    result.forEach(e => {
                        customer = {
                            id: e.id,
        ...
        app.post("/crm/createCustomer", (req, res) => {
            ...
            models.customers.create({
                ...
                if (err) {
                    ...
                    models.login.create({ username: user, password: pass, customer_id: result.id }, (err, result1) => {
                        if (err) {
                            result.remove((err) => {
                                console.log('add customer failed ' + id)
                                res.sendStatus(403)
                            ...
                            models.account.create({ customer_id: result1.customer_id, amount: 0 }, (err, result2) => {
                                if (err) {
                                    result1.remove((err) => {
                                        result.remove((err) => {
                                            console.log('add customer failed ' + id)
        ...
        app.get("/crm/editCustomer/:id", (req, res) => {
            if (checkUser(req.session, res, 'crm')) {
                models.customers.get(req.params.id, (err, customer) => {
                    if (err) {
                        res.sendStatus(404)
                    } else {
        ...
        app.post("/crm/editCustomer/:id", (req, res) => {
            ...
            const gender = (req.body.gender === "1")
            const ssn = req.body.ssn
            models.customers.get(req.params.id, (err, result) => {
                if (err) {
        ...
        app.get("/crm/loanList", (req, res) => {
            if (checkUser(req.session, res, 'crm')) {
                var loans = []
                var count = 0
                models.loan.find(true, (err, result) => {
                    if (err) {
                        res.sendStatus(403)
                    } else {
                        count = result.length
                        if (count === 0) {
                            ...
                            result.forEach(e => {
                                ...
                                models.customers.get(e.customer_id, (err, result) => {
                                    if (err) {
                                        res.sendStatus(403)
                                    } else {
        ...
        app.post("/crm/createLoan", (req, res) => {
            ...
            models.customers.get(cus_id, (err, result) => {
                if (err) {
        ...
        app.get("/crm/loanEdit/:id", (req, res) => {
            if (checkUser(req.session, res, 'crm')) {
                models.loan.get(req.params.id, (err, result) => {
                    if (err) {
                        res.sendStatus(403)
                    } else {
                        loan = {
            ...
            if (checkUser(req.session, res, 'crm')) {
                models.customers.get(req.params.id, (err, result) => {
                    if (err) {
                        ...
                        models.account.get(req.params.id, (err, result) => {
                            if (err) {
                                customer.balance = 0
                            } else {
                                customer.balance = result.amount
                            }
                            models.loan.find({ customer_id: customer.id }, (err, result) => {
                                if (err) {
                                    res.render('crm/DepositWithdraw', { customer: undefined })
                                } else {
                                    result.forEach(e => {
        ...
        app.post("/crm/Dow", (req, res) => {
            ...
            const amount = +req.body.amount
            models.account.get(id, (err, result) => {
                if (err) {
                    console.log(err)
                    res.sendStatus(403)
                } else {
                    if (dow) {
                        ...
                        result.save((err) => {
                            if (err) {
                                ...
                                const timestamp = Date.now()
                                const date = formatDate(Date(timestamp)) + ' ' + formatTime(Date(timestamp))
                                models.transaction.create({ customer_id: id, date: date, amount: amount }, (err, result) => {
                                    if (err) {
                                        res.sendStatus(403)
                                    } else {
                                        res.sendStatus(200)
        ...
        app.get(["/debt", "/debt/toDoList"], (req, res) => {
            if (checkUser(req.session, res, 'debt')) {
                var loan_own = []
                var loans = []
                models.tracking.find({debt_id: req.session.user.officer_id},(err,result)=>{
                    if (err) {
                    ...
                    models.loan.find((loan_own.length !== 0)? {id: orm.not_in(loan_own)}:true, (err, result) => {
                        if (err) {
                            res.render('dept/ToDoList', { loans })
                        } else {
        ...
        app.post("/debt/toDoList", (req, res) => {
            ...
            req.body.loans.forEach(id => {
                models.tracking.create({ loan_id: id, debt_id: req.session.user.officer_id }, (err, result) => {
                    count++   
                    ...
                    if (err) {
                        ...
                        if (result.length === 0) {
                            ...
                            result.forEach(e => {
                                ...
                                models.loan.get(e.loan_id, (err2, result2) => {
                                    loan.amount = result2.amount
                                    models.customers.get(result2.customer_id, (err3, result3) => {
        ...
        app.get(["/customer", "/customer/information"], (req, res) => {
            if (checkUser(req.session, res, 'customer')) {
                var customer = {}
                models.customers.get(req.session.user.customer_id, (err, result) => {
                    if (err) {
                        res.sendStatus(403)
                    } else {
                        models.account.get(result.id, (err, result2) => {
                            customer.balance = 0
                            if (err) {
                                res.sendStatus(401)
                            } else {
                            ...
                            models.loan.find({ customer_id: result.id }, (err, result3) => {
                                customer.dept = 0
                                if (err) {
                                    res.sendStatus(401)
                                } else {
        ...
        app.get("/customer/transaction", (req, res) => {
            if (checkUser(req.session, res, 'customer')) {
                var transactions = []
                var count = 0
                models.transaction.find({ customer_id: req.session.user.customer_id }, (err, result) => {
                    if (err) {
